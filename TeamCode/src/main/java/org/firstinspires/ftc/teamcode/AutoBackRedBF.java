...
public class AutoBackRedBF extends OpMode {
    ...
    public void init() {
        ...
        limelight3A.pipelineSwitch(6);//1 is green
    ...
    public void loop() {
        if (done) {
            ...
            rtIntake.setPower(0.75);
            drive.drive(0,0,0);
            //!\\straghten out (heading -90)----------------------------------------------------------------------
            int goodloops = 0;
            while(goodloops < 5){
                if(odo.getHeading(AngleUnit.DEGREES) < -88 && odo.getHeading(AngleUnit.DEGREES) > -92){
                    drive.drive(0,0,0);
                    goodloops++;
            ...
            rtIntake.setPower(1);//!\\\ change on blue
            rtFire.setPower(0);
            odo.update();
            double ypos = odo.getPosX(DistanceUnit.CM);
            while(abs(ypos-odo.getPosX(DistanceUnit.CM)) < 2){//!!\\ ADJUST SLIDE HYAH
                odo.update();
            ...
            while(abs(xpos-odo.getPosY(DistanceUnit.CM)) < 30){//was 30 change on blue
                odo.update();
                if(odo.getHeading(AngleUnit.DEGREES) > -90){
