...
@Disabled
@TeleOp
public class TeleOpRedxx extends OpMode {
    ...
    private int idleSpeed = 620;
    double shootOfsetValue = 0;
    ...
    public void loop(){
        ...
        if(gamepad1.right_bumper) { //TARGET
            ...
            if(rot == -1){//cant see the tag or other problem
                ...
                else if(rot < 20 & (rot > (2+shootOfsetValue))){ drive.drive(0,strafe,-targetSpeedLow);}
                else if((rot <=(2+shootOfsetValue))  & (rot >= (-2+shootOfsetValue))){ drive.drive(0,strafe,0);}
                else if(rot > -20 & (rot < (-2+shootOfsetValue))){ drive.drive(0,strafe,targetSpeedLow);}
                //else if(rot <= -20){drive.drive(0,0,-targetSpeedMed);}
                else{drive.drive(0,strafe,0);}//catchall
            }
        }else if(gamepad1.left_bumper && (Math.abs(rot) <= 2)){
            //fire line up
        ...
        int TargetVelocity;
        if(distance > 130){
            TargetVelocity = (int) (idleSpeed + ((distance-165)*1));//was 860- down town shooting
        }else if(distance < 130 & distance > 60){
            TargetVelocity = (int) (idleSpeed + ((distance - 60)*0.84)); //set the intermediate power orignal 580 dist 84 was 74 70 was .66
        }else if(distance < 60){
            TargetVelocity = idleSpeed;//original 580
        }else{
            TargetVelocity = idleSpeed;//original 630
        }
        if(distance >= 130){//calibrating for distance
            shootOfsetValue = -3;
        }else{
            shootOfsetValue = 0;
        }
        ...
        if(gamepad2.left_bumper && rot < 2 && rot > -2 && (!(rot == -1)) && shooterMotor2.getVelocity() > (TargetVelocity - 30)){

            ballStopLeft.setPosition(0.7);
        }else{
            ballStopLeft.setPosition(0.25);
        }
        if(gamepad2.right_bumper && rot < 2 && rot > -2 && (!(rot == -1)) && shooterMotor.getVelocity() > (TargetVelocity - 30)){
