...
public class AutoFrontBlue99 extends OpMode {
    ...
    public void loop() {
        if(done){
            ...
            int goodlops = 0;
            while(goodlops < 12){//lign up
                odo.update();
                if(odo.getHeading(AngleUnit.DEGREES) < 2 && odo.getHeading(AngleUnit.DEGREES) > -2){
                    goodlops++;
                    drive.drive(0,0,0);
                }else{
                    goodlops = 0;
            ...
            while(abs(ypos-odo.getPosY(DistanceUnit.CM)) < 40){//strafe
                odo.update();
                if(odo.getHeading(AngleUnit.DEGREES) > 0){
            ...
            goodlops = 0;
            while(goodlops < 12){//line up again
                odo.update();
                if(odo.getHeading(AngleUnit.DEGREES) < 2 && odo.getHeading(AngleUnit.DEGREES) > -2){
                    goodlops++;
                    drive.drive(0,0,0);
            ...
            ltFire.setPower(0.2);//was .3
            drive.drive(-0.1,0,0);
            while(color.getDistance(DistanceUnit.CM) > 3.4){//collect first ball
            ...
            while(abs(ypos-odo.getPosY(DistanceUnit.CM)) < 6){//strafe a bit
                odo.update();
                if(odo.getHeading(AngleUnit.DEGREES) > 0){
                    drive.drive(0,0.2,-0.1);
                }else{
            ...
            while(!((velocity >= 660 && velocity <=680) && (shooterMotor2.getVelocity() >= 680 && shooterMotor2.getVelocity() <= 680))){//wait for shooters to be at speed
                velocity = shooterMotor.getVelocity();
                telemetry.addData("speed:",velocity);
                if(shooterMotor.getVelocity() <= 660){
                    shooterMotor.setPower(1);
            ...
            ltIntake.setPower(0);
            ypos = odo.getPosY(DistanceUnit.CM);
            while(abs(ypos-odo.getPosY(DistanceUnit.CM)) < 70){
                odo.update();
                if(odo.getHeading(AngleUnit.DEGREES) > 0){
                    drive.drive(0,0.5,-0.25);
                }else{
